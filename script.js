...
function injectNotificationBell() {
    ...
    bell.style.cssText = 'position:fixed;bottom:20px;left:20px;width:50px;height:50px;background:#524c43;color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:1000;font-size:1.2rem;';
    bell.innerHTML = '<i class="fas fa-bell"></i>';
    document.body.appendChild(bell);
    bell.onclick = () => alert('Stay tuned for the latest tech news! 🔔');
}
...
function initNotificationPopup() {
    ...
    pop.innerHTML = `<div class="notify-popup" style="background:#fff;padding:30px;border-radius:12px;text-align:center;max-width:350px;"><div style="font-size:2rem;color:#0066cc;margin-bottom:15px;"><i class="fas fa-bell"></i></div><h3>Get Daily Updates</h3><p>Allow notifications to never miss tech news.</p><div style="display:flex;gap:10px;justify-content:center;margin-top:20px;"><button class="btn-deny" style="padding:8px 20px;cursor:pointer;border:1px solid #ddd;border-radius:5px;">Not Now</button><button class="btn-allow" style="padding:8px 20px;background:#0066cc;color:#fff;border:none;border-radius:5px;cursor:pointer;">Allow</button></div></div>`;
    document.body.appendChild(pop);
    pop.querySelector('.btn-deny').onclick = () => { localStorage.setItem('notify_status', 'denied'); pop.remove(); };
    pop.querySelector('.btn-allow').onclick = () => { localStorage.setItem('notify_status', 'allowed'); pop.remove(); };
}
...
function initLiveSearchSystem() {
    ...
    overlay.innerHTML = `<div class="search-modal"><div class="search-header"><i class="fas fa-search" style="margin-right:10px;color:#888;"></i><input type="text" id="main-search-input" placeholder="Search articles..."><i class="fas fa-times close-search-btn" style="cursor:pointer;margin-left:10px;color:#888;"></i></div><div id="search-results-list" style="max-height:400px;overflow-y:auto;"></div></div>`;
